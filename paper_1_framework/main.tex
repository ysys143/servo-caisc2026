...
A non-trivial result emerges from varying the prior seeding rate: at low prior coverage (seeding rate $\leq 5\%$, each agent covering $\approx 22\%$ of latent classes), more connected topologies achieve better balanced accuracy (lattice BA $=0.563$, $t=-7.4$ vs.\ isolated at $p<0.001$); but at moderate prior coverage (seeding rate $\geq 10\%$, each agent covering $\approx 39\%$ of classes), isolated agents achieve \emph{higher} balanced accuracy ($0.564$, $t=+3.1$ vs.\ lattice at $p<0.01$), because additional memory sharing over-saturates collective knowledge and collapses recall for genuinely novel hypotheses. This reversal is structurally consistent with the epistemic-network literature~\citep{zollman2010epistemic,rosenstock2017epistemic}: the advantage of sparse connectivity is greatest when true positives are rare, and diminishes as individual coverage grows. The simulation thereby illustrates one minimal mechanism by which $V$ incompleteness produces systematic failure in autonomous scientific loops---and suggests that optimal validator connectivity is prior-knowledge-dependent rather than monotonically determined by completeness alone.
