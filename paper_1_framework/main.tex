...
\textbf{Aletheia}~\citep{google2026aletheia} takes the opposite approach: $S$ is natural language proof space, $V$ is a fallible LLM Verifier subagent, and $V_\text{human}$ remains the ground truth. Of 700 Erd\H{o}s problems attempted, 200 solutions were graded by human auditors, of which 13 (6.5\%) were rated ``meaningfully correct.'' The system explicitly identifies ``subconscious plagiarism''---pretraining data contamination masking as discovery---as a serious validity threat.
...
\textbf{GNoME}~\citep{merchant2023gnome} (Graph Networks for Materials Exploration, DeepMind) scales this structure to 2.2 million candidate inorganic crystal structures. $G$ is a pair of Graph Neural Networks (GNNs) trained to predict thermodynamic stability: one structural GNN operating on known crystal prototypes and one compositional GNN operating on element combinations. $E$ is high-throughput DFT. $\pi$ is deep-ensemble uncertainty sampling: candidates where the GNN ensemble disagrees most are prioritized for DFT evaluation, approximating an active learning policy over $b(S)$. $M_s$ grows as the GNN retrains on accumulated DFT outcomes; GNoME is one of the few systems where $G(M_s, S)$ genuinely improves with accumulated experiments.
...
Component & Instantiation \\
\midrule
$S$ & 2.2M candidate inorganic crystal structures \\
...
$\pi$ & Uncertainty sampling: prioritize high-uncertainty (deep-ensemble disagreement) predictions for DFT \\
\bottomrule
\end{tabular}
...
To make the consequences of $V$ incompleteness concrete, we construct a minimal agent-based simulation in which the degree of $V$ completeness is the only variable altered across conditions. We model a community of $n=20$ \emph{reviewer agents}, each holding a local memory of previously observed latent classes. Each agent acts as a local validator: it estimates novelty by checking whether an incoming hypothesis belongs to a latent class it has encountered before, subject to a classifier accuracy parameter that models imperfect semantic identification. The hypothesis space consists of $n=200$ hypotheses drawn uniformly from 40 latent classes; ground-truth novelty is determined by first occurrence of each class (exactly 40 hypotheses are novel, 160 are redundant). Agents share memory only through their communication topology, which we vary as the primary independent variable.
...
High-connectivity conditions suppress false novelty at the cost of recall: the shared condition achieves near-zero FNR ($0.008 \pm 0.012$) but also near-zero true novelty recall ($0.029 \pm 0.025$), because a saturated collective memory classifies genuinely novel hypotheses as already known. No condition operates as a useful discriminator: all conditions fall within 0.07 of chance-level balanced accuracy (BA range 0.506--0.565 at the default seeding rate). The structural reason is that a coverage threshold has only one degree of freedom and cannot improve false-novelty rejection and true-novelty detection simultaneously.

A non-trivial structural consequence emerges from varying the prior seeding rate ($n=10$ seeds per condition): at low prior coverage (seeding rate $\leq 5\%$, each agent covering $\approx 22\%$ of latent classes on average), connected topologies achieve higher balanced accuracy than isolated (lattice BA $= 0.563$ vs.\ isolated $0.501$ at $5\%$ seeding); but at moderate prior coverage (seeding rate $\geq 10\%$, each agent covering $\approx 39\%$ of classes), isolated agents achieve higher balanced accuracy ($0.564$ vs.\ lattice $0.529$ at $10\%$). The mechanism is not Bayesian belief dynamics but a geometric one: both topologies exhibit recall declining and specificity rising with seeding rate, but connectivity accelerates memory saturation, shifting the peak of each topology's BA curve to a lower seeding rate. The reversal marks where the isolated topology's BA peak has not yet been reached while the connected topology has already entered over-saturation. Qualitatively, this is directionally consistent with the observation that memory sharing can suppress genuine novelty detection when individual validators already hold substantial coverage---but the model contains no informative-signal dynamics and the analogy to formal epistemic-network models~\citep{zollman2007communication,zollman2010epistemic} is phenomenological only. The simulation illustrates one minimal mechanism by which $V$ incompleteness produces systematic failure in autonomous scientific loops, and shows that optimal validator connectivity depends on the community's prior knowledge state.
