...
\textbf{Results.} Figure~\ref{fig:fnr} reports the false novelty rate (FNR: fraction of non-novel hypotheses incorrectly marked as novel) across connectivity conditions and classifier accuracies, averaged over 30 random seeds. The connectivity effect is dominant: FNR drops from $0.955 \pm 0.028$ (isolated, $k=0$) to $0.285 \pm 0.047$ (lattice, $k=2$) to $0.075 \pm 0.036$ ($k=4$) to $0.008 \pm 0.012$ (shared, $k=n{-}1$). Within each connectivity condition, varying classifier accuracy from 0.60 to 0.90 changes FNR by at most $0.035$ in absolute terms, while the FNR range across connectivity conditions is $0.947$ at fixed accuracy---a 27-fold difference between the structural and individual-quality effects. Note that high-connectivity conditions suppress false novelty at the cost of recall: the shared condition achieves near-zero FNR but also near-zero true novelty recall ($0.029 \pm 0.025$), because a highly saturated collective memory tends to classify genuinely novel hypotheses as already known. Balanced accuracy (mean of recall and specificity) ranges from 0.51 (isolated: indiscriminate positive) to 0.50 (shared: indiscriminate negative), confirming that neither extreme operates as a useful discriminator; intermediate connectivity conditions achieve the best balanced discrimination.

\begin{figure}[h]
\centering
\includegraphics[width=0.72\columnwidth]{fig_fnr_primary_n30.pdf}
\caption{False novelty rate (FNR) by $V$-completeness level (connectivity condition) and classifier accuracy. Error bars show $\pm 1$ std across 30 seeds ($n=200$ hypotheses, 40 latent classes, 20 agents). Connectivity effect (FNR range $0.947$ at fixed accuracy) is 27$\times$ larger than the classifier-accuracy effect (FNR range $0.035$ at fixed connectivity).}
\label{fig:fnr}
\end{figure}

An additional robustness sweep varying the hypothesis space complexity ($n_\text{latent} \in \{20, 40, 80\}$) reveals that the magnitude of $V$ incompleteness scales with domain complexity: isolated agents achieve FNR $=0.642$ when only 20 latent classes exist (a simple domain where local memory frequently saturates), but FNR rises to $0.995$ at 80 classes. Medium-connectivity ($k=4$) deteriorates similarly (FNR $0.002 \to 0.292$), while shared topology remains near zero across all complexity levels. This illustrates one minimal mechanism by which $V$ incompleteness produces systematic failure in autonomous scientific loops---and why the failure mode is more severe in complex domains such as biology and mathematics (Table~\ref{tab:domain-comparison}) than in constrained settings such as formal verification.

\textbf{Limitations.} This simulation uses synthetic agents and a ground-truth novelty oracle unavailable in real science; its agents do not generate hypotheses, run experiments, or update beliefs. The connectivity-to-$V$-completeness mapping is an operationalization, not an identity: the mechanism (memory union growth with degree) is structurally determined rather than emergent, and the reported FNR-recall tradeoff is an analytic consequence of the coverage-based estimator rather than an independent empirical finding. The effect magnitude depends on the prior seeding rate (fraction of literature each agent has pre-read): the topology ordering is robust across seeding rates of 2--20\%, but the absolute FNR difference collapses as seeding rates increase beyond 10\%. The simulation is intended only to make the structural consequence of $V$ incompleteness legible, not to measure its magnitude in any deployed system.
