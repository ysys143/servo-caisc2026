...
\textbf{Operationalizing $V$ completeness via topology.} Each topology represents a different degree of $V$ completeness: an isolated agent can only consult its own memory (an incomplete, purely local $V$); agents on a lattice gain access to immediate neighbors (partial $V$); small-world connectivity adds long-range links that improve coverage; and a fully shared topology gives every agent access to the entire community's collective memory (the most complete $V$ in this setting). Table~\ref{tab:sim-conditions} formalizes this mapping.

\begin{table}[h]
\centering
\small
\caption{Simulation conditions. Each topology operationalizes a different level of $V$ completeness.}
\label{tab:sim-conditions}
\begin{tabular}{@{}llll@{}}
\toprule
Topology & $V$-completeness level & Graph structure & Effective coverage \\
\midrule
Isolated    & Incomplete ($V$-local)     & No edges           & Agent's own memory only \\
Lattice     & Partial                    & Ring ($k=2$)       & 2-hop neighborhood \\
Small-world & Improved                   & Watts--Strogatz ($k=4, p=0.3$) & Local + long-range links \\
Shared      & Complete (in this setting) & Complete graph     & Full community memory \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Results.} Figure~\ref{fig:fnr} reports the false novelty rate (FNR: fraction of non-novel hypotheses incorrectly marked as novel) across topologies and classifier accuracies, averaged over 10 random seeds. The topology effect is dominant: FNR drops from $0.961 \pm 0.021$ (isolated) to $0.300 \pm 0.041$ (lattice) to $0.064 \pm 0.035$ (small-world) to $0.008 \pm 0.009$ (shared)---a 120-fold reduction from most to least incomplete $V$. In contrast, varying classifier accuracy from 0.60 to 0.90 within the isolated condition changes FNR by only $0.040$ in absolute terms. The $V$ structure effect is approximately 24$\times$ larger than the individual agent quality effect, consistent with the claim that $V$ completeness is a more consequential design variable than the precision of individual components.

\begin{figure}[h]
\centering
\includegraphics[width=0.72\columnwidth]{fig_fnr_primary.pdf}
\caption{False novelty rate (FNR) by $V$-completeness level (topology) and classifier accuracy. Error bars show $\pm 1$ std across 10 seeds ($n=200$ hypotheses, 40 latent classes, 20 agents). The topology effect dominates: the FNR range across topologies (0.953) is $\approx$24$\times$ the range across classifier accuracies (0.040).}
\label{fig:fnr}
\end{figure}

An additional robustness sweep varying the hypothesis space complexity ($n_\text{latent} \in \{20, 40, 80\}$) reveals that the magnitude of $V$ incompleteness scales with domain complexity: isolated agents achieve FNR $=0.642$ when only 20 latent classes exist (a simple domain where local memory frequently saturates), but FNR rises to $0.995$ at 80 classes. Small-world topology deteriorates similarly (FNR $0.002 \to 0.292$), while shared topology remains near zero across all complexity levels. This illustrates one minimal mechanism by which $V$ incompleteness produces systematic failure in autonomous scientific loops---and why the failure mode is more severe in complex domains such as biology and mathematics (Table~\ref{tab:domain-comparison}) than in constrained settings such as formal verification.

\textbf{Limitations.} This simulation uses synthetic agents and a ground-truth novelty oracle unavailable in real science; its agents do not generate hypotheses, run experiments, or update beliefs. The topology-to-$V$-completeness mapping is an operationalization, not an identity: real validators differ from reviewer networks in important ways. The simulation is intended only to make the structural consequence of $V$ incompleteness legible, not to measure its magnitude in any deployed system.
