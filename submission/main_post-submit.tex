...
\begin{description}
  ...
  \item[$\pi: b \times M \rightarrow h$: Search Policy] The decision rule for selecting the next hypothesis $h \in S$. The belief state $b$ is updated after each experiment. When the next decision encodes a design $d$, a myopic BED rule may be written $\pi_{\mathrm{EIG}}(b,M) \in \arg\max_{d\in\mathcal D}\mathrm{EIG}(d)$ (Equation~\ref{eq:eid}); this is a one-step action rule, not a globally optimal sequential policy. In practice, current systems use policies ranging from reactive (ReAct) to greedy to tree search.
\end{description}
...
\noindent\textbf{Validator dimensions rather than a reliability order.} The labels $V_\text{syntax}$, $V_\text{semantic}$, $V_\text{empirical}$, $V_\text{human}$, and $V_\text{formal}$ identify different target properties and are not ordered measurements of one reliability variable. A deterministic syntax check can be exact for executability while saying nothing about novelty or scientific significance; a proof kernel can be exact for formal validity while leaving provenance and importance unresolved. We therefore characterize a system by a vector: the operational gate (if identified), its target property, evidence of bias or calibration, experimental fidelity, and external independence. The earlier holistic $V$-completeness ordinal is retained only as a disclosed exploratory annotation in Appendix~\ref{app:coding}; it is not an explanatory variable or a substantive ordering in this paper. The binary released field $V_\text{gating}=1$ means that an identified decisive acceptance gate has evidence of calibration in the protocol's weak sense of no documented systematic bias. A value of 0 means only that this criterion is not established; it does not distinguish no gate, a non-gating assessment layer, an unidentified gate, and an identified but uncalibrated gate. Those cases require the accompanying role description. No surveyed system reports probabilistic calibration (reliability curves, expected calibration error, Brier score), so that stronger property is neither claimed nor coded. Appendix~\ref{app:vformal} gives bounded analytical observations that keep the observation, reward, and novelty channels distinct.
...
We apply \framework{} to the core systems in Table~\ref{tab:core-comparison} and one artifact infrastructure proposal, reporting validator roles alongside loop status, $G$, $E$, $M$, and $\pi$. The small convenience sample does not identify an ordering or association among those dimensions.
...
\midrule
Math (formal) & $V_\text{formal}$ & Exact (formal) & Formal eval. only & RL/MCTS  & Pretraining contamination \\
...
\bottomrule
\end{tabular}
\caption{Illustrative \framework{} domain mapping across seven fields (per-domain coding in \texttt{domain\_systems.csv}). For formal mathematics, ``Exact (formal)'' means exact for formal validity and ``Formal eval. only'' means mechanically closed candidate evaluation within the formalized task; neither covers novelty, importance, provenance, or the discovery process as a whole. Other $V$ reliability entries are property-specific ordinal labels, not measured agreement with a common ground truth. Domain codings are single-coder interpretations of the cited primary sources; no inter-coder $\kappa$ statistics are available for this table.}
...
Equation~\ref{eq:eid} specifies a one-step, myopic information criterion, yet no surveyed LLM-based system implements it: all deployed LLM-based policies are heuristic (ReAct, greedy, tree search, human-guided). The partial exception is GNoME~\citep{merchant2023gnome}, whose deep-ensemble uncertainty estimates over a tractable GNN posterior can be read as a coarse EIG surrogate---but only because DFT provides a well-calibrated computational $V$ and the hypothesis space admits a learnable probabilistic model, conditions absent from the LLM-based systems here, where the ``posterior'' is implicit in model weights~\citep{foster2021dad}. BED-LLM~\citep{choudhury2026bedllm} derives a usable EIG posterior from an LLM's predictive distributions, but for conversational queries rather than experiment design---so the gap is integration with reliable $E$ and $V$, not impossibility. Concretely, every LLM-based system in Table~\ref{tab:core-comparison} uses a heuristic $\pi$: Coscientist selects tools reactively, The AI Scientist (Sakana) filters ideas by archive similarity (which we code as greedy; the source states no selection objective), and Agent Laboratory follows a fixed schedule---none implements an explicit posterior-updating information objective. Recent falsification-driven approaches ground hypothesis selection in executable tests rather than free generation---AIGS selects hypotheses by their survival of automated refutation~\citep{liu2024aigs}, and Popper runs sequential falsification experiments with statistical error control~\citep{huang2025popper}---but neither computes or maximizes EIG: they substitute a falsification heuristic for the BED criterion rather than realizing it.
...
Architecturally, with $\theta$ implicit in frozen weights and no per-experiment update, a system must either maintain an explicit probabilistic surrogate (GNoME's route, viable only where a calibrated input--outcome model exists) or use amortized EIG estimation~\citep{foster2021dad}, which remains unsolved for open-ended $\mathcal{S}$. Part of this gap is moreover conceptual: EIG-based selection presupposes an already-represented hypothesis space, whereas how genuinely new hypotheses or representations \emph{arise} falls outside current BED formulations~\citep{yanai2019night}---a generative limitation complementary to the channel-based non-identification discussed in Appendix~\ref{app:vformal}.
...
This problem is structurally distinct from Op1 and Op2: any $V$ can evaluate only evidence at the fidelity it receives, and an EIG-based $\pi$ can select only among the experiments represented at the currently available fidelity---so without $E$ escalation criteria the system can close a computational loop indefinitely without reaching the physical evidence trustworthy scientific claims require.
...
\textbf{Validator design as a diagnostic vocabulary.} Unreliable validation can distort what a system records as progress---an ecosystem-scale symptom is the documented surge in fabricated citations~\citep{zhao2026hallucinations}, and strong task performance need not imply an internalized world model~\citep{vafa2025foundationmodel}. The survey does not isolate $V$ from $G$, $E$, $M$, and $\pi$, and its outcome coding is partly entangled with $V$; it therefore does not establish even the direction of a cross-system association. The framework instead asks property-specific questions: what operational gate is used, what property does it assess, what bias or calibration evidence exists, at what experimental fidelity, and with what external independence?

\textbf{Human oversight.} Rather than treating $H \in [0,1]$ as a quantity to minimize, we read explicit human integration at the $V_\text{human}$ and $\pi$ levels as appropriate for the surveyed systems: their automated proxies do not yet provide independently validated novelty and significance judgments. This is a current-workflow finding, not a claim that those judgments are non-automatizable in principle. The goal is not full autonomy but \emph{appropriate} autonomy.
...
\textbf{Limitations.} \framework{} abstracts away several factors: the cost structure of $E$ (orders of magnitude across domains), the non-stationarity of $S$, and the single-agent framing's social layer~\citep{taniguchi2024cpc}. The surveyed systems are a convenience sample subject to selection and survivorship bias, and mechanical loop closure does not require reliable acceptance feedback. Trustworthy closure is not measured independently of $V$, so the present data cannot test its association with gate calibration. The coding methodology and multi-vendor reliability estimates are in Appendix~\ref{app:coding}; the historical holistic $V$-completeness ordinal has only fair agreement ($\kappa{=}0.39$) and is excluded from substantive claims. Using LLM coders to characterize systems whose validators may themselves be unreliable is a reflexive limitation; property-specific human expert coding remains necessary future work.

\section{Conclusion}
\label{sec:conclusion}

We introduced \framework{} \sgmvpi{}, connecting AI Scientist systems to POMDP and BED theory across six core systems and seven domains. Its analytical distinctions support only narrow diagnostic claims, and the proposed association between gate calibration and trustworthy closure remains untested. \framework{} names property-specific questions about validator roles, search-policy design, and experiment-fidelity escalation without compressing them into a scalar completeness order.
...
\begin{enumerate}
  ...
  \item \textbf{Analysis of data and interpretation of results.}
    ...
    Explanation: The LLM agent extracted \framework{} components for all analyzed systems from primary sources and performed citation-level fact verification (4 verification sweeps). The human author reviewed all extractions, corrected mischaracterizations, and synthesized the original V-completeness thesis, which R20 withdraws from substantive analysis in favor of property-specific dimensions. Consistent with this venue's premise, the cross-system coding is AI-conducted under human direction and disclosed as such; its rigor rests on auditability (released rubric, per-row source quotes, and regeneration/citation-validation scripts) and conservative claim-scoping rather than an independent human gold standard, which remains necessary future work.
...
\item {\bf Claims}
    ...
    \item[] Justification: The abstract and introduction explicitly state the three contributions (framework, cross-domain analysis, open problems). The bounded analytical observations in \S\ref{app:vformal} clarify the separation of observation, reward validation, and novelty judgment rather than establishing a V-completeness theorem, and the cross-domain survey is presented as illustration; the paper delimits both the statistical conditions and the survey's illustrative role in \S\ref{app:vformal} and \S\ref{sec:discussion}.
...
\item {\bf Broader impacts}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: \S\ref{sec:discussion} addresses both the potential for accelerating scientific discovery and the risk of premature convergence under widespread AI Scientist deployment (particularly via the experiment fidelity gap and the current lack of validated automated novelty gates).
...
Three independent, model-pinned, blind LLM-based coder agents from different vendors coded 14 Tier-1 systems (see \texttt{multicoder/target\_systems.md}): Claude Code (\texttt{claude-opus-4-8}, Anthropic), Codex CLI (\texttt{gpt-5.5}, OpenAI), and Antigravity (\texttt{gemini-3.1-pro}, Google), each run headless and model-pinned, with exact invocations and per-model outputs released in the supplement. The six core systems in Table~\ref{tab:core-comparison} are a representative analytical sample spanning the discovery-loop spectrum. Inter-coder agreement (Fleiss' $\kappa$ over the 13 systems coded by all three vendors---NovelSeek drew only two usable codings after a coder timeout and is excluded) is substantial-to-perfect on most constructs (calibration $\kappa{=}0.79$, loop status $\kappa{=}0.74$, semantic-validation presence $\kappa{=}1.0$; mean $\kappa{=}0.68$) and only fair on the historical holistic $V$-completeness ordinal ($\kappa{=}0.39$). Agreement between the coders and the author's reference labels was originally negative on that ordinal ($-0.25$ to $-0.43$); a later audit found part of that gap to be author coding error rather than construct noise, and after correction it is $0.17$ to $0.67$ (Appendix~\ref{app:postsubmit}). The coder-only figures quoted here are unaffected by that correction. All are automated transparency statistics, not human-level reliability. Because the ordinal compresses distinct properties---gate identity and calibration, property coverage, experimental fidelity, and external independence---we disclose it for audit history but exclude it from substantive analysis in favor of the vector-valued description.

A retrospective counterexample search found no surveyed system coded as achieving independently established trustworthy closure without reliable acceptance feedback, but the outcome labels overlap $V$ and cannot test that association. A two-vendor recode over all 14 systems (supplement) resolves a \emph{present}-vs-\emph{gating} ambiguity---for AI Scientist (Nature~2026), the automated reviewer is present but non-gating (raw agreement 13/14, Cohen's $\kappa{=}0.86$; a robustness check, not expert-validated reliability). Here $V_\text{gating}=0$ records failure to establish a calibrated decisive gate, not proof that no operational gate exists. The role description identifies the automated reviewer as an assessment instrument and leaves the actual acceptance mechanism uncalibrated or unresolved. Introduced post-hoc, this split reflects iterative construct development rather than a prospective reliability study.
...
\paragraph{Mathematics, formal ($V_\text{formal}$; mechanically closed formal evaluation).} FunSearch~\citep{romera2023funsearch} and DeepSeek-Prover~\citep{xin2024deepseek} validate against a formal or numeric checker---an evaluator function or proof kernel returning a deterministic pass/fail. This $V_\text{formal}$ provides deterministic formal-correctness feedback and mechanically closes candidate evaluation within the formalized task. It does not validate novelty, importance, provenance, or the discovery process as a whole. The residual bottleneck is not formal correctness but $S$: pretraining contamination makes it hard to certify that a discovered result is genuinely novel rather than memorized---the formal-domain instance of the channel-based novelty problem discussed in Appendix~\ref{app:vformal}.

\paragraph{Mathematics, natural language ($V_\text{sem}+V_h$; closed at low quality).} Aletheia~\citep{google2026aletheia} produces natural-language proofs scored by LLM and human judgment. The loop closes mechanically, but $V_\text{sem}$ is imperfect: of 200 audited Erd\H{o}s solutions only 13 (6.5\%) were rated meaningfully correct. Replacing the formal oracle with a semantic one reintroduces human review and exposes hallucination and plagiarism as failure modes. This is evidence about the reliability of the reward or acceptance channel; without a specified normalized update likelihood it is not an instance of the fixed-i.i.d. coupling analysis in Appendix~\ref{app:vformal}. The Leiden Declaration~\citep{leiden2026} corroborates the practical limitation: AI-produced natural-language proofs may contain almost-invisible errors that resist detection without expert scrutiny. This supports continued expert review in current workflows, not a theorem that human validation is irreplaceable in every possible future system.
...
\paragraph{Cross-domain reading.} In the illustrative coding, observed bottlenecks vary with $V$ type alongside the other framework components: formal oracles support mechanically closed evaluation within formalized tasks; imperfect semantic or empirical validators co-occur with hallucination, circular-validity, or insufficient-metric failures; and expensive empirical evaluation in materials science co-occurs with a resource bottleneck. This is the diagnostic use of Table~\ref{tab:domain-comparison}: it does not claim a tested cross-domain law or isolate a causal effect of $V$, but shows that the same $(S,G,E,V,M,\pi)$ vocabulary localizes where each domain's loop breaks. $V$ type is one factor associated with trustworthy closure, while the search policy $\pi$ separately bears on which represented regions of $S$ the loop explores. Table~\ref{tab:domain-comparison} captures the $V$ dimension of this provisional two-axis reading; the $\pi$ dimension remains underdeveloped across the surveyed domains, with materials science a partial exception.
...
\begin{description}
  ...
  \item[R11. NovelSeek / InternAgent citation identity (Table~\ref{tab:core-comparison}).]
...
made since the archival deposit, R1--R20. Entries R3--R12 narrow or correct statements
...
design. These changes do not alter the six-component framework, the three open
problems, or the status of the cross-system survey as
provisional structured annotation. The bibliography is unchanged apart from the single entry
corrected in R11. Should this manuscript be revised further, later entries will be appended
to the list above so that the full distance from the version of record remains reconstructible
