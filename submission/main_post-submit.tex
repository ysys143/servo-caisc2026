...
Discovery-cycle feedback & An evidence occurrence & \texttt{epistemic\_update}, an explicit \texttt{epistemic\_action} event on the feedback-controlled route, execution at $E$, then \texttt{observation} back to $V$ & A second distinct evidence occurrence; endpoint-only action inference, retry-only, append-only memory, and terminal-only paths do not qualify \\
\bottomrule
\end{tabular}
\end{table*}
...
contracts finalized through R33--R46. References in earlier entries to ``final'' Schema~1 tables or current
