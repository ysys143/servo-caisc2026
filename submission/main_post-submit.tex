...
An established predicate requires an ordered witness whose events, edges, endpoints, and artifacts remain within the same case, version, configuration, and task regime. Discovery-cycle feedback has the strongest contract: an evidence event must lead through an epistemic-update edge to a feedback-dependent epistemic action, a new evidence-generating execution, and a new evidence event. Missing or ambiguous source links yield \emph{unknown} or \emph{not established}; they are not completed by architectural plausibility.
...
Schema~2.0's normative files are the case, endpoint, artifact, event, edge, reliability-evaluation, closure-witness, closure-status, domain-anchor, and selection-ledger records named in \texttt{servo\_schema.yaml}. R24 did not code these units and remains historical development-audit material only.
...
The Robot Scientist is a negative case against an automatic claim of added diagnostic value: its source already exposes the relevant hypothesis, assay, update, and selection sequence~\citep{king2004robot,sparkes2010robot}. Servo supplies a portable record format but does not thereby discover a hidden mechanism. The examples show only what the schema foregrounds.
...
\textbf{Limitations.} \framework{} is non-exhaustive and abstracts away agent-level and social organization~\citep{taniguchi2024cpc}. The six cases and seven anchors are selected source-traceable records, not representative or population-valid samples. A predicate status concerns only its bounded case and witness; it does not imply scientific outcome quality. Servo supplies neither a causal model nor a generative law for missing routes, costs, fidelity transitions, or artifacts. R24 predates Schema~2.0 and is retained only as a historical development audit; its agreement coefficients do not establish reproducibility, source entailment, construct validity, human reliability, real-world utility, or comparative superiority.

\section{Conclusion}
\label{sec:conclusion}

We introduced \framework{} \sgmvpi{} as a non-exhaustive, source-traceable event-and-evidence schema. Its six versioned cases from five lineages and seven frozen anchors illustrate how to record evaluation events, artifact versions, routes, authority, and five conservative closure predicates. They do not establish prevalence, causality, comparative superiority, or scientific outcome effects.
...
\item {\bf Claims}
    ...
    \item[] Justification: The abstract and introduction state the non-exhaustive Schema~2.0 contribution, its six bounded cases and seven frozen anchors, and the prospective open questions. Appendix~\ref{app:vformal} supplies bounded analytical observations rather than a new theorem, while the case and anchor records are explicitly non-representative source annotations.
...
\item {\bf Experimental result reproducibility}
    ...
    \item[] Justification: This paper contains no computational experiment, but its main case annotations and closure predicates depend on the Schema~2.0 package. A complete local package exists, including six versioned cases, seven frozen anchors, selection, provenance, event, artifact, edge, reliability, and closure-witness records. It has not yet been independently published at a verified public repository or DOI and therefore is not yet externally reproducible from the official record.

\item {\bf Open access to data and code}
    \item[] Question: Does the paper provide open access to data and code?
    \item[] Answer: \answerNo[Not yet]
    \item[] Justification: The working tree contains Schema~2.0 and its case, endpoint, artifact, event, edge, reliability, closure-witness, closure-status, domain-anchor, and selection-ledger records. This local availability is not open access. No public repository snapshot or DOI for the complete Schema~2.0 package has been verified, and the package is not part of the archival version of record.
...
R24 froze its then-current manual and obtained 42 accepted fresh-session codings, three vendors for each of 14 source records. It is retained only as a historical development audit. Its record-level token unions, Jaccard summaries, MASI-based Krippendorff $\alpha$, and disagreement log neither align Schema~2.0 events and edges nor reproduce its closure witnesses. No R24 statistic is transferred to the current contract or case statuses.
...
made since the archival deposit, R1--R31. Entries R3--R12 narrow or correct statements
...
R31 records that the complete Schema~2 working package exists locally but is unpublished and
not part of the official record, and changes the reproducibility and open-access checklist
answers accordingly.
