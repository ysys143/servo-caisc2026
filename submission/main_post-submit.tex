...
\begin{description}
  ...
  mechanism, and the cell is flagged for recoding to ``established'' with an explicit witness once the
  witness chain is added; the conservative status is retained only pending that witness rather than on
  the ground of missing source support.
  ...
  general executability validator. This revision records the overstatement and flags the anchor for
  restriction to protocol-planning (pseudocode reconstruction), with the wet-lab run to be marked as a
  single external validation rather than a recurring execution channel in the synchronized recoding.
\end{description}
...
experimental-adaptation cell is coded conservatively against direct source evidence and flags it for
witnessed recoding; R69 restricts the BioPlanner anchor from an overstated laboratory/executability
channel to its actual pseudocode-reconstruction benchmark. None of these entries changes the
framework, the three open problems, or their proposed evaluations.
