...
POMDP, NovelSeek, CPC-MS, and ARA remain complementary rather than direct survey-taxonomy baselines. POMDP supplies a sequential decision model when states and kernels are specified~\citep{kaelbling1998pomdp}; NovelSeek foregrounds implemented multi-agent organization~\citep{zhang2025novelseek}; CPC-MS models community-level epistemic dynamics~\citep{taniguchi2024cpc}; and ARA foregrounds artifact provenance and replay~\citep{liu2026lasthuman}. Servo does not subsume these analytical units.
...
\noindent\textbf{Portable event--artifact graph.}
...
for each of five predicates: \emph{execution repair}, \emph{experimental adaptation},
\emph{artifact revision}, \emph{discovery-cycle feedback}, and \emph{human-mediated feedback}.
An established predicate requires an ordered witness whose events, edges, endpoints, and artifacts remain within the same case, version, configuration, and task regime. Execution repair requires runtime failure evidence, artifact revision through $W_A$, and a later execution; experimental adaptation requires an epistemic-update and feedback-control route to a later evidence-generating execution; artifact revision requires a successor versioned artifact; discovery-cycle feedback further requires new evidence after an intervening epistemic action and execution; and human-mediated feedback requires an in-workflow human-feedback event and routed mediation edge. The predicates may share records only when the path independently satisfies each contract.
...
The frozen application contains six versioned cases from five lineages: Coscientist; AI Scientist 2024 and AI Scientist 2026 as two versions in one lineage; Agent Laboratory; Robot Scientist; and NovelSeek/InternAgent. Each case is bounded by a reported configuration and task regime. The records are selected source-traceable examples, not ``core'' representatives of a population. They describe $S,G,E,V,M,\pi$, the boundary objects $O_{\mathrm{env}},A,W_A$, human authority, endpoints, events, artifacts, routed edges, reliability evaluations, and any closure witnesses that satisfy the Servo event--artifact contract.
...
We introduced \framework{} \sgmvpi{} as a non-exhaustive, source-traceable event-and-evidence schema. Its six versioned cases from five lineages and seven frozen anchors illustrate how to record evaluation events, artifact versions, routes, authority, and five conservative closure predicates. The schema reuses generic provenance concepts while adding scientific target, reliability, authority, epistemic-route, and closure-witness semantics. These formative cases and partial anchors do not establish out-of-sample portability, prevalence, causality, comparative superiority, or scientific outcome effects.
...
\item {\bf Experimental result reproducibility}
    ...
    \item[] Answer: \answerNo[Not yet]
    \item[] Justification: This paper contains no computational experiment, but its main case annotations and closure predicates depend on the Servo event--artifact package. A complete local successor package exists, including six versioned cases, seven frozen anchors, selection, provenance, event, artifact, edge, reliability, closure-witness, predicate-contract, and provenance-crosswalk records. A prior public snapshot exists at \url{https://github.com/ysys143/servo-caisc2026/releases/tag/servo-corrected-v2.0.0}, but it predates the clarified predicate/status contract and is not evidence for the revised status matrix. The synchronized successor package is therefore not yet externally reproducible from an immutable public release.

\item {\bf Open access to data and code}
    \item[] Question: Does the paper provide open access to data and code?
    \item[] Answer: \answerNo[Not yet]
    \item[] Justification: The prior public snapshot above remains available, but the working tree now contains a materially clarified contract and revised closure statuses that it does not contain. Until a successor immutable release binds the current case, endpoint, artifact, event, edge, reliability, closure-witness, closure-status, domain-anchor, selection-ledger, contract, code, tables, and manuscript files, open access to the current analysis is incomplete.
...
\begin{description}
  ...
  language is removed, and the multi-agent limitation now states that systems such as NovelSeek
  ...
  acknowledging NovelSeek's advantage for agent organization and the Robot Scientist as a
  ...
  Wei et al. and Tie et al.; POMDP, NovelSeek, CPC-MS, and ARA remain complementary adjacent
...
made since the archival deposit, R1--R32. Entries R3--R12 narrow or correct statements
