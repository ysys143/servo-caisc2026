...
- **두 반사실 각각이 독립적으로 >=2 사례의 해석을 바꾼다:**
  ...
  2. **adaptive-selection 분해(BED 측) — v3 7축 재도출(후보-축 + 설계-축 분리).** BED 유도 6축(`control_dependence`·`selection_signal`·`selection_objective`·`candidate_selection_rule`·`design_selection_rule`·`candidate_execution_rule`)을 벗기면 여섯 사례가 하나의 "결과 조건부 적응 루프(outcome-conditioned adaptive loop)"로 병합한다. 축이 있을 때 C05(`hypothesis_model_discrimination`+후보 `exhaustive`/`all_selected`+설계 `fixed_or_standard_design`/`coverage_or_factorial`)는 성능-클러스터(`performance_improvement`+`top_k_ranked`/`threshold`)와 **구별되고**, C01(`local_repair`, diversity 없음, `sequential_choice`+`until_success`)은 diversity/score 클러스터와 **구별된다.** 설계 축은 여기에 실험-설계 차원을 더한다 — C03/C05만 `coverage_or_factorial`(체계적 sweep·ablation / Latin-square)이고 나머지는 `fixed_or_standard_design`이다(margin은 C03+C05로 좁다, 정직 기록). 축을 벗기면 이 서로 다른 피드백 구조가 하나의 서술로 합쳐진다 — >=2 사례(실측상 3: C05, C01, 보강 C04)의 해석이 실제로 바뀐다. **reviewer Item 2 교정:** C05의 "20개 가설 전수 시험"은 **후보(h)-선택** 사실(`candidate_selection_rule=exhaustive`/`candidate_execution_rule=all_selected`)일 뿐, 실험-DESIGN을 기대판별로 고르는지에 대한 증거가 아니다 — 설계 축은 별개로 표준/coverage이며 `discrimination_directed`가 아니다(king2004의 기대비용-최소 실험선택은 인용 배경, 동결 sparkes2010 C05 출처 밖). 이 distinction 2 판정은 §4.2/4.3의 with/without-축 반사실만으로 성립하며, §4.4(퇴역 closure 표현)는 관측된 병합이 아닌 **표현적 공백**을 지적하는 하중 없는 교정 주석이다.
...
값 = v3 enum 그대로. `generation_scope`는 provenance 전용(표 A 열 아님)이나 붕괴 검증을 위해 함께 싣는다. `obs_result`=`observation_result`, `perf_impr`=`performance_improvement`, `div_dir`=`diversity_directed_selection`, `uncred`=`uncertainty_reduction`, `disc`=`hypothesis_model_discrimination`, `fixed_pre`=`fixed_or_predefined`, `fixed_std`=`fixed_or_standard_design`, `cov_fac`=`coverage_or_factorial`. **`design_selection_rule`(reviewer Item 2)는 candidate_selection_rule과 별개 표 A 열이다** — 후보 h 선택과 실험 설계 d 선택을 분리한다.
...
| C05 | fixed_pre, score, obs_result | **uncred, disc** | fixed_space | **exhaustive** | **fixed_std, cov_fac** | **all_selected** |
...
| C05 | **uncred, disc** | **exhaustive** | **fixed_std, cov_fac** | **all_selected** | fixed_or_predefined | 가설구별/불확실성 목적, **20개 가설 전수 후보선택·전수 실행**(후보 하선택 없음), **설계는 표준 assay + Latin-square coverage(판별-지향 아님)**, repair 없음 | 결과 조건부 적응 루프 |
...
축을 벗기면 >=2 사례(C05, C01; C04·설계축 보강)의 해석이 실제로 합쳐진다. **반사실 2 판정: load-bearing.** 판별 기준은 **어휘 존재가 아니라 사례 INTERPRETATION의 변화**이며, 위 사례들 모두 축 유무에 따라 독법이 실제로 바뀐다.
...
- *부분적으로 강하나 전체로는 실패.* **양보:** 실제로 채워진 셀 값들(`performance_improvement`/`local_repair`/`top_k_ranked`/`threshold`/`sequential_choice`)은 대체로 일반 최적화 어휘로도 진술 가능하고, 정보-지향 값들(`uncertainty_reduction`/`hypothesis_model_discrimination`)은 약하게만 발화한다(C05 목적 한 곳, 그마저 후보 rule은 `exhaustive`/`all_selected`이지 후보 기대판별 하선택 아니고, 설계 축도 표준+coverage이지 `discrimination_directed` 아님). **그러나 실패하는 이유:** 하중은 셀 값의 어휘가 아니라 **여섯 사례를 서로 다르게 읽게 만드는 판별 행위**에 있다. "결과조건부인가 `fixed_or_predefined`인가? 목적이 성능향상인가 가설구별인가? 후보를 `exhaustive`로 고르는가 랭크 하선택인가? 실험 설계 d를 표준/coverage로 고르는가 판별-지향으로 고르는가?"를 모든 사례에 던지는 것이 BED의 기여이며(후보 h 선택과 설계 d 선택을 별개로 물음, reviewer Item 2), 이 질문을 제거하면(§4.2/4.3 반사실) 여섯 사례가 하나의 결과-조건부 루프로 합쳐져 C05(구별·전수·표준coverage설계)와 C01(다양화-없음·순차)의 독법이 소멸한다 — 골격은 분석적 일을 한다. 장식 아님. (퇴역 4-predicate matrix는 이 질문을 담을 축이 아예 없어 C05 구조를 표현조차 못 했으나, 이는 관측된 병합이 아니라 표현적 공백일 뿐이고 §4.4대로 하중 증거가 아니다.)
...
- **distinction 2 (adaptive-selection 분해): load-bearing, >=2 사례 — v3 7축(후보-축 + 설계-축 분리)으로 재도출된 붕괴 테스트.** BED 유도 6축(`control_dependence`/`selection_signal`/`selection_objective`/`candidate_selection_rule`/`design_selection_rule`/`candidate_execution_rule`)을 벗기면 여섯 사례가 하나의 "결과 조건부 적응 루프"로 병합되며, C05의 구별목적+전수 후보(`exhaustive`/`all_selected`) 구조가 성능-클러스터로, C01의 다양화-없는 순차 착취 독법이 다양성/랭크 클러스터로 흡수된다. **reviewer Item 2:** 실험-설계 축(`design_selection_rule`)이 후보-축과 분리되어, C05의 "20개 가설 전수 시험"이 곧 "실험 판별 없음"이라는 혼동을 제거한다 — C05의 설계는 별개로 표준+coverage(`fixed_or_standard_design`/`coverage_or_factorial`, discrimination_directed 아님; king2004는 인용 배경)이며, 설계 축은 C03·C05만 coverage로 갈려 distinction 2를 보강한다(단독 margin은 C03+C05로 좁음, 정직 기록). margin은 C05+C01에 집중(정직 기록). 이 하중은 §4.2/4.3 반사실만으로 성립하고, §4.4(퇴역 closure 표현)는 관측된 병합이 아닌 표현적 공백을 지적하는 교정 주석으로 하중 밖이다.
