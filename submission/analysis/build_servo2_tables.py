...
ANCHOR_LABELS = {
    ...
    "DA05": (("BioPlanner", "BioPlanner"), ("Biology", "생물학"), ("Protocol-planning pseudocode reconstruction", "프로토콜 계획 의사코드 재구성"), ("Generator; pseudocode evaluator; one-off lab check", "생성기·의사코드 평가기·1회 실험 확인"), ("Diagnostic benchmark; single validation", "진단 벤치마크; 단일 검증"), ("Protocol; single lab run", "프로토콜; 단일 실험실 실행")),
...
def validate(cases: list[dict[str, str]], closure: list[dict[str, str]], anchors: list[dict[str, str]], witnesses: list[dict[str, str]], ledger: list[dict[str, str]]) -> None:
    ...
    if any(row["schema_version"] != "4.1.0" for row in cases + closure + anchors + witnesses + ledger):
