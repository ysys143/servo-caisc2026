...
\label{prop:suff}
Under Assumption~\ref{asm:tract} the posterior concentrates on $\theta^\star$ (the loop closes). If in addition $V$ is calibrated, the one-step EIG rule (Equation~\ref{eq:eid}) is myopically Bayes-optimal for the information objective.
\end{proposition}

\noindent\emph{Proof sketch.} A well-specified, identifying likelihood with \emph{divergent} accumulated information on a compact $\Theta$ (Assumption~\ref{asm:tract}(i),(iv),(v)) is the standard sufficient condition for Bayesian posterior consistency~\citep{ghosal2017fundamentals}, giving $b_t \to \delta_{\theta^\star}$. Calibration is not required for consistency; it only makes one-step EIG myopically optimal, by definition of expected information gain. The uniform-information clause~(v) is essential: a sequence with $\mathrm{EIG}(d_t)>0$ but $\sum_t \mathrm{EIG}(d_t)<\infty$ leaves the posterior short of a point mass even under identifiability and calibration. $\square$
