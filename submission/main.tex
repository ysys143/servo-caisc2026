...
\author{
  Jaesol Shin\thanks{Prepared as a submission to the Conference for AI Scientists (CAISc)~2026. Fitting the venue's theme, the work was itself produced with substantial AI assistance: drafting and component analysis used Claude Code (Anthropic), and the cross-system reliability coding was performed by three model-pinned LLM agents from different vendors (Anthropic, OpenAI, Google; see \S\ref{sec:discussion} and the supplement). The human author provided conceptual direction, framework design, source verification, and final judgment. See the AI Involvement Checklist for full details.}\\
  \small{WeDataLab}\\
...
\textbf{Design hierarchy.} Our analysis suggests $V$ design is a necessary constraint on closed-loop operation: unreliable validation prevents a system from learning whether its outputs represent progress. The surveyed systems do not isolate $V$ from $G$, $E$, $M$, and $\pi$---Coscientist and The AI Scientist also advance planning, tool use, and tree-based experimentation---so we read $V$ completeness as a recurring bottleneck, not an established causal priority. The practical implication still stands: before optimizing $G$ or $\pi$, designers should ask what level of $V$ completeness the domain actually supports.
...
\textbf{Limitations.} \framework{} abstracts away several important factors. The cost structure of $E$ varies by orders of magnitude across domains (milliseconds for code execution, hours for DFT, weeks for wet-lab) and is not modeled. The non-stationarity of $S$ (open-ended problems) is treated as a property to be noted but not formalized. The single-agent framing abstracts away the social layer that \citet{taniguchi2024cpc} formalize; extending \framework{} to multi-agent settings is a natural direction for future work. Finally, the cross-domain comparison relies on publicly reported system descriptions; component characterizations may not capture all implementation details. More fundamentally, the $V$-completeness pattern is a cross-system \emph{association}, not an established causal determinant, and the surveyed systems are a convenience sample subject to selection and survivorship bias. Mechanical loop closure moreover does not itself require completeness (AI Scientist v1 and NovelSeek close a computational loop at low completeness), so the claim concerns \emph{trustworthy} closure---which we do not measure independently of $V$; we therefore present the validator/trustworthiness link as an organizing hypothesis, not a tested association. The supplement adds a transparency check that codes a separate \texttt{trustworthy\_closure} outcome from signals external to the validator (independent replication, post-hoc human audit, documented hallucination) and reports a descriptive contingency that is direction-consistent but far too small (six systems) for inference. To make the coding auditable we release an operational rubric, a machine-readable coding sheet with a representative source quote and resolvable citation per system, and a validator script that rejects any uncited or unquoted row. Three independent, model-pinned, blind LLM-based coder agents from different vendors (Anthropic, OpenAI, Google)\footnote{Coders: Claude Code (\texttt{claude-opus-4-8}, Anthropic), Codex CLI (\texttt{gpt-5.5}, OpenAI), and Antigravity (\texttt{agy}, Gemini~3.1~Pro, high reasoning; Google), each run headless and model-pinned; exact invocations and per-model outputs are released in the supplement.} coded 14 systems; inter-coder agreement (Fleiss' $\kappa$ over the 13 jointly-coded systems) is substantial-to-perfect on most constructs (calibration $\kappa{=}0.79$, loop status $\kappa{=}0.74$, semantic-validation presence $\kappa{=}1.0$; mean $\kappa{=}0.68$) and only fair on the holistic $V$-completeness ordinal ($\kappa{=}0.39$, on which independent coders also disagree with the author), reported as an automated multi-rater transparency measure rather than human-level reliability. A counterexample search found no system that is $V$-incomplete yet achieves \emph{trustworthy} closed-loop discovery: the lone structural near-miss (AI Scientist v1) closes its loop on an admittedly biased validator but yields unreliable output. The pattern is thus made precise, under stated assumptions, by Propositions~\ref{prop:suff}--\ref{prop:novelty}, with calibration---its most robustly coded sub-construct---as the decisive correlate.

\section{Conclusion}
\label{sec:conclusion}

We introduced \framework{} \sgmvpi{}, a formal framework connecting AI Scientist systems to POMDP and BED theory, and applied it to four core systems, one artifact infrastructure proposal, and seven scientific domains. The central pattern in our author-coded survey is that greater validator reliability tends to coincide with trustworthy closed-loop discovery; we read this as an observational association, not a proven causal hierarchy. Propositions~\ref{prop:suff}--\ref{prop:novelty} support only narrower claims---closure in the tractable subproblem requires a well-specified, identifying observation model and sufficient accumulated information (validator calibration governs the reward/policy channel, not posterior consistency), and novelty is unidentified only when no automated channel carries novelty information---so the formalism motivates separating observation, reward validation, and human novelty judgment rather than proving that $V$ completeness is the primary determinant across real systems.

The decomposition makes implicit comparisons explicit: formal mathematics is most mature because $V_\text{formal}$ is binary-reliable; chemistry and biology stay loop-open because $V$ depends on physical measurement; and no system compounds knowledge across domains because transferable $M_s$ is absent (GNoME improves only within its trained chemical space). These are structural constraints, not gaps closable by better language models alone. The three open problems are neither independent nor equally tractable---ARA's Rigor Auditor and GNoME's uncertainty sampling show partial progress---suggesting the bottleneck is missing architecture, not impossibility. \framework{} offers the vocabulary to name what is needed: validators and memory structures both complete enough to close the loop and structured enough to support principled search.
