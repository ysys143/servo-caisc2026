...
Table~\ref{tab:domain-comparison} maps \framework{} instantiations across seven scientific domains; $V$ completeness appears to co-vary with loop closability across the surveyed domains, consistent with Propositions~\ref{prop:suff}--\ref{prop:novelty}. To illustrate scale: Aletheia~\citep{google2026aletheia} attempted 700 Erd\H{o}s problems; of the 200 solutions selected for human audit, 13 (6.5\%) were rated meaningfully correct; GNoME~\citep{merchant2023gnome} predicted 2.2 million structures (380{,}000 stable) using DFT-in-the-loop active learning. Systems analyzed span mathematics (formal:~\citep{romera2023funsearch,xin2024deepseek}; NL:~\citep{google2026aletheia}), physics~\citep{udrescu2020afeynman,cranmer2020symbolic}, chemistry~\citep{bran2023chemcrow,sprueill2024chemreasoner}, materials~\citep{merchant2023gnome}, biology~\citep{odonoghue2023bioplanner}, social science~\citep{manning2024automated,aher2023turing,park2023generative}, and CS/algorithms~\citep{real2020automlzero,sartori2024algodiscovery,jaber2026autokernel}.
...
\noindent The three open problems are not independent. A system with complete $V$ and tractable $M_s$ would have the prerequisites for an EIG-optimal $\pi$: complete $V$ gives the calibrated reward EIG requires, and tractable $M_s$ gives the hypothesis-space model over which EIG is computed. Whether these suffice in non-stationary or unbounded $S$ remains open, but they are necessary, so the problems converge on a single structural gap where progress on one unlocks gains toward the others. They are also precisely the necessary conditions for a collective AI co-scientist~\citep{taniguchi2024cpc}: $V_\text{collective}$ requires solved novelty automatability; a principled collective $\pi$ requires closed BED--practice gap; and the shared semantic memory $w$ requires developed $M_s$. ARA~\citep{liu2026lasthuman} may be a plausible entry point, simultaneously externalizing $M_s$ and calibrating the sub-novelty layers of $V$.
...
\item {\bf Experimental result reproducibility}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: This paper contains no computational experiments. The cross-domain analysis approach (system selection and component extraction from primary sources) is described in \S\ref{sec:core} and \S\ref{sec:domains}; a machine-readable extraction sheet with a representative source quote per system, explicit inclusion/exclusion criteria, multi-vendor blind coding by three model-pinned LLM raters over 14 systems with Fleiss' $\kappa$, a counterexample search, and a table-regeneration/validation script is released as supplementary material (see \S\ref{sec:discussion}). All analyzed papers are publicly accessible.
